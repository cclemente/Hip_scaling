...
setwd("/data/Dropbox/PostDoc/Manuscripts/HipHeight/hip-scaling-with-david")
data<-read.csv("mass_eff_speed_4.csv", row.names="name")
data$name = rownames(data)
#List of species excluded by peter
Peter.ex <- data$name[is.na(data$name2)==T]
#Subset of the species for which there is speed data
Speed.data <- data$name[which(data$speed3>0)]
#Species excluded by Peter which have speed data
Both.ex <- Speed.data[Speed.data %in% Peter.ex]

data$color <- rep("black",length(data$name))
data$color[match(Peter.ex,data$name)] <-  "Blue"
data$color[match(Both.ex,data$name)] <-  "Red"

colors2 <- rep("black", Nedge(pruned))
colors2[which(pruned$edge[,2] %in% 1:501)] <- data$color
plot(pruned, edge.color=colors2)

myo<-read.nexus("nature05634-s2.txt")
myo1<-myo[[1]]
# prune tree by the species in mydata
matches<-name.check(myo1,data)
pruned<-drop.tip(myo1,matches$tree_not_data)

plot(pruned, type="fan", show.tip.label = F, edge.color=colors2, edge.width=2)
