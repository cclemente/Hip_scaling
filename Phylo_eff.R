...
# plot it - easy to see that theres no significant effect of diet on the residual distribution.
...
plot(fit)
graphics.off()
plot(eff~log10(mass),mydata, pch=20, col=cols[mydata$diet], font.lab = 2, las = 1, ylab = expression(bold(paste("Effective fore-limb length"))), xlab = expression(bold(paste("Log10(body mass)"))))
abline(fit, col="red", lwd=2) # add a red abline to the data with 'fit's information
# There's an upward trend- a slope of 0.08, and the summary(lm) indicates that this is significant (p < 2.2e-16)
